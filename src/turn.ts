...
export async function runUserTurn(
  ...
  // 3. 只追加这一条新 user；日志是 append-only，绝不能把整个上下文重存一遍。
  ...
  // 5. step() 前检查上下文压力。压缩失败时返回原上下文，不影响本轮继续执行。
  const prepared = await compactIfNeeded(sessionId, state, userMsg);
  const fullContext: ChatMessage[] = [systemMsg, ...prepared.contextMessages, userMsg];

  // 6. step() 原地推进 fullContext，并把 token / 工具事件透传给上层。
  const before = fullContext.length;
  await step(fullContext, toolRegistry, 10, handlers);

  // 7. 只有 step 之后新增的消息需要追加。
  const added = fullContext.slice(before);
  for (const msg of added) {
    await appendMessage(sessionId, msg);
  }

  // 8. 最终回复 = 最后一条 assistant 的文字内容。
